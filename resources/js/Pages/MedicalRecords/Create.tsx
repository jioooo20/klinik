...
export default function Create({ patient, icd10Options }: CreateProps) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        vitals: { sistolik: '', diastolik: '', suhu: '', nadi: '', respirasi: '' },
        prescription: '',
    });

    const submit = (e: FormEvent) => {
        e.preventDefault();
        post(route('patients.records.store', patient.id));
    };
    ...
    return (
        <MedicalRecordsLayout>
            <Head title={`Rekam Medis Baru — ${patient.name}`} />
                ...
                <CardContent>
                    <form onSubmit={submit} className="space-y-5">
                        ...
                        <div className="flex gap-2">
                            <Button type="submit" disabled={processing}>Simpan Rekam Medis</Button>
                            <Link href={route('patients.records.index', patient.id)}>
